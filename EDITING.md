...
This is a native Quarto website project. The authoritative book text is in the `.qmd` files in this folder. You can edit the same files in VS Code or RStudio.

## Open in your editor

- **VS Code:** open this folder and install the recommended Quarto extension. Open a chapter `.qmd` file. Use the terminal command below to preview the whole site. Turn on **View → Word Wrap** for comfortable paragraph editing.
- **RStudio:** open `missing_data.Rproj`, then open a chapter `.qmd` file. Use the Terminal tab for `quarto preview`, or render the project with `quarto render`. The project configuration also applies when previewing a chapter. Source editing is recommended to preserve the existing equation labels and complex tables.

| File | What to edit |
| --- | --- |
| `index.qmd` | Front-page text and author information |
| `chapter-1.qmd` … `chapter-7.qmd` | Chapter prose and mathematics |
...
```powershell
quarto preview --port 8261
```

Open <http://127.0.0.1:8261/>. Keep the terminal running. Save a chapter file; Quarto rebuilds it and refreshes the preview. After changing shared navigation metadata in `book.json`, restart the preview or run `quarto render` to update all pages. Press Ctrl+C to stop the preview. The public website is unaffected until you push.
...
Blocks marked `::: math-block`, `::: math-passage`, or `::: book-figure` preserve the chapter layout. Edit the prose and equations inside them and retain the matching colon fences. Figure image tags and a few complex tables retain HTML to preserve their existing layout; ordinary prose and equations use Markdown. Images are relative to the published site root, for example `figures/MARid.png`.
...
Quarto renders the `.qmd` files directly. Its configured post-render script (`scripts/finish-site.py`) applies the existing custom design and checks local links. You do not run that script manually. Python and the dependency in `requirements.txt` support this styling step; R is not required to render the current text-only chapters.
